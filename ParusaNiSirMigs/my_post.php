<?php

session_start();

mysqli_report(MYSQLI_REPORT_ERROR | MYSQLI_REPORT_STRICT);

include "config.php";


if (!isset($_SESSION['user_id'])) {

    header("Location:login.php");

    exit();

}


$user_id = $_SESSION['user_id'];
$__initial = strtoupper(substr($_SESSION['username'] ?? '?', 0, 1));

/* get user posts */
$query = mysqli_query(

    $conn,

    "
    SELECT *

    FROM posts

    WHERE user_id='$user_id'

    ORDER BY created_at DESC
    "

);

?>

<!DOCTYPE html>
<html lang="en">

<head>

    <meta charset="UTF-8">

    <meta
        name="viewport"
        content="width=device-width, initial-scale=1.0"
    >

    <title>My Posts - BLOG APP</title>

    <link
        rel="stylesheet"
        href="style.css"
    >

</head>

<body>


<?php include "navbar.php"; ?>


<div class="container">

    <div class="page-header">

        <h2>MY POSTS</h2>

        <a
            href="add_post.php"
            class="primary-link"
        >

            + New Post

        </a>

    </div>


    <?php if (isset($_GET['success'])): ?>

        <p class="message">

            <?= htmlspecialchars(
                $_GET['success']
            ); ?>

        </p>

    <?php endif; ?>


    <?php if (isset($_GET['deleted'])): ?>

        <p class="message">

            <?= htmlspecialchars(
                $_GET['deleted']
            ); ?>

        </p>

    <?php endif; ?>


    <!-- composer -->
    <a href="add_post.php" class="composer">

        <div class="avatar"><?= htmlspecialchars($__initial); ?></div>

        <span class="composer-input">
            What's on your mind, <?= htmlspecialchars($_SESSION['username'] ?? ''); ?>?
        </span>

    </a>


    <?php if (mysqli_num_rows($query) == 0): ?>


        <!-- EMPTY POSTS -->

        <div class="empty-message">

            You don't have any posts yet.

        </div>


    <?php else: ?>


        <?php while ($post = mysqli_fetch_assoc($query)): ?>


            <div
                class="post-card"
                id="post-<?= $post['id']; ?>"
            >

                <div class="post-header">

                    <div class="post-meta">

                        <div class="avatar"><?= htmlspecialchars($__initial); ?></div>

                        <div class="post-meta-text">

                            <b class="post-author"><?= htmlspecialchars($_SESSION['username'] ?? 'You'); ?></b>

                            <small class="post-info">
                                <?= htmlspecialchars($post['created_at']); ?>
                                <?php if ($post['updated_at'] && $post['updated_at'] !== $post['created_at']): ?>
                                    &nbsp;&middot;&nbsp;edited
                                <?php endif; ?>
                            </small>

                        </div>

                    </div>


                    <!-- edit/delete -->
                    <div class="post-actions">

                        <a
                            href="edit_post.php?id=<?= $post['id']; ?>"
                            class="edit-link"
                        >

                            Edit

                        </a>

                        <a
                            href="delete_post.php?id=<?= $post['id']; ?>"
                            class="delete-link"
                            onclick="return confirm(
                                'Are you sure you want to delete this post?'
                            );"
                        >

                            Delete

                        </a>


                    </div>


                </div>

                <h3>

                    <?= htmlspecialchars(
                        $post['title']
                    ); ?>

                </h3>

                <p>

                    <?= nl2br(
                        htmlspecialchars(
                            $post['body']
                        )
                    ); ?>

                </p>


                <?php

                $post_id = $post['id'];


                /* count comments */
                $comment_query = mysqli_query(

                    $conn,

                    "
                    SELECT COUNT(*) AS total_comments

                    FROM comments

                    WHERE post_id='$post_id'
                    "

                );

                $comment_data = $comment_query
                    ? mysqli_fetch_assoc($comment_query)
                    : ['total_comments' => 0];

                $total_comments =
                    $comment_data['total_comments'];


                /* count likes */
                $like_count_query = mysqli_query(

                    $conn,

                    "
                    SELECT COUNT(*) AS total_likes

                    FROM likes

                    WHERE post_id='$post_id'
                    "

                );

                $like_data = $like_count_query
                    ? mysqli_fetch_assoc($like_count_query)
                    : ['total_likes' => 0];

                $total_likes =
                    $like_data['total_likes'];


                /* check user like */
                $like_check = mysqli_query(

                    $conn,

                    "
                    SELECT id

                    FROM likes

                    WHERE post_id='$post_id'

                    AND user_id='$user_id'
                    "

                );

                $is_liked =
                    $like_check && mysqli_num_rows($like_check) > 0;

                ?>

                <?php if ($total_likes > 0 || $total_comments > 0): ?>

                    <div class="post-stats">

                        <?php if ($total_likes > 0): ?>
                            <span class="stat-likes">
                                <span class="like-dot">👍</span>
                                <?= $total_likes; ?>
                            </span>
                        <?php endif; ?>

                        <?php if ($total_comments > 0): ?>
                            <span class="stat-comments">
                                <?= $total_comments; ?>
                                <?= ($total_comments == 1) ? ' comment' : ' comments'; ?>
                            </span>
                        <?php endif; ?>

                    </div>

                <?php endif; ?>

                <div class="like-share-section">

                    <!-- like -->
                    <div class="like-section">

                        <form
                            action="like_post.php"
                            method="POST"
                        >

                            <input
                                type="hidden"
                                name="post_id"
                                value="<?= $post['id']; ?>"
                            >


                            <button
                                type="submit"
                                class="like-button
                                <?= $is_liked ? 'liked' : ''; ?>"
                            >

                                <svg viewBox="0 0 24 24" width="18" height="18" fill="<?= $is_liked ? 'currentColor' : 'none'; ?>" stroke="currentColor" stroke-width="2" stroke-linejoin="round">
                                    <path d="M7 22V11l6-9 1.5 1.5L13 10h7a2 2 0 0 1 2 2.4l-1.6 7A2 2 0 0 1 18.4 21H10a3 3 0 0 1-3-3z"></path>
                                </svg>

                                <?= $is_liked ? 'Liked' : 'Like'; ?>

                                <span class="like-count">

                                    <?= $total_likes; ?>

                                </span>

                            </button>

                        </form>

                    </div>


                    <!-- share -->
                    <div class="share-section">

                        <button
                            type="button"
                            class="share-button"
                            onclick="sharePost(
                                '<?= htmlspecialchars(
                                    $post['title'],
                                    ENT_QUOTES
                                ); ?>',
                                '<?= htmlspecialchars(
                                    'http://' .
                                    $_SERVER['HTTP_HOST'] .
                                    dirname($_SERVER['PHP_SELF']) .
                                    '/community.php#post-' .
                                    $post['id'],
                                    ENT_QUOTES
                                ); ?>'
                            )"
                        >

                            <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linejoin="round" stroke-linecap="round">
                                <path d="M4 12v7a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-7"></path>
                                <path d="M16 6l-4-4-4 4"></path>
                                <path d="M12 2v13"></path>
                            </svg>

                            Share

                        </button>

                    </div>


                </div>


            </div>


        <?php endwhile; ?>


    <?php endif; ?>


</div>

<script>

function sharePost(title, url) {

    if (navigator.share) {

        navigator.share({

            title: title,

            url: url

        }).catch(function () {

            console.log("Share cancelled.");

        });

    }

    else {

        navigator.clipboard.writeText(url)

        .then(function () {

            alert("Post link copied!");

        })

        .catch(function () {

            alert("Unable to copy the post link.");

        });

    }

}

</script>

</body>

</html>